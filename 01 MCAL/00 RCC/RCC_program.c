...
{
// turning on clocks
	if (RCC_HSI_clk == 1 )SET_BIT(RCC_CR, HSION); while(!(RCC_CR & (1<<HSIRDY)));
	if (RCC_HSE_clk == 1 )SET_BIT(RCC_CR, HSEON); while(!(RCC_CR & (1<<HSERDY)));
// turning on PLL clock
	if (RCC_PLL_clk == 1 ){
//setting PLL entry clock source and HSE's entry value to PLL
		if (RCC_PLL_SRC == 1 )SET_BIT(RCC_CFGR, PLLSRC);
		if (RCC_PLL_XTPRE == 1 )SET_BIT(RCC_CFGR, PLLXTPRE);
...
// turning on clock security and HSE clock bypass
	if (RCC_CSS == 1 )SET_BIT(RCC_CR, CSSON);
	if (RCC_HSE_BYP == 1 )SET_BIT(RCC_CR, HSEBYP);
//setting system clock
	if (RCC_SYS_CLK == RCC_PLL) SET_BIT(RCC_CFGR, SW1);
	if (RCC_SYS_CLK == RCC_HSE) SET_BIT(RCC_CFGR, SW0);



	//choosing USB prescaler
		if (RCC_USB_PRE == 1 )SET_BIT(RCC_CFGR, USBPRE);
